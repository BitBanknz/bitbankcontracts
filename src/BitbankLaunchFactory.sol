...
// Bitbank adaptation of the Pons Family V1 factory.
// Changes: require 1% V3 pool fee / tick spacing 200; exact CREATE2 salts (no vanity loop).
// Original source attribution and third-party licenses retained.
// Pons Family — launchpad factory for https://pons.family
// Upstream Pons deployment reference only: 0xA5aAb3F0c6EeadF30Ef1D3Eb997108E976351feB.
// This Bitbank adaptation has no deployment address.
...
import {PonsLauncherToken} from "./vendor/pons-v1/PonsLauncherToken.sol";
import {PonsLiquidityMath} from "./vendor/pons-v1/libraries/PonsLiquidityMath.sol";
 ...
 * @author Pons Family
 * @notice Bitbank V1 entrypoint, adapted from the Pons Family launchpad.
 ...
 * route leftover native value into the first buy. Upstream Pons reference:
 ...
 */
 ...
 // Initial purchased tokens belong to the creator, never the collaborator fee splitter.
 address initialBuyRecipient = msg.sender;
 ...
 (bytes32 deploySalt,) =
 _resolveSalt(salt, initCodeHash, config.pairToken, dex.factory, dex.poolFee);
 ...
 */
 ...
 /// @dev Use the caller's exact salt; vanity address mining wastes launch gas.
 function _resolveSalt(bytes32 startSalt, bytes32 initCodeHash, address pairToken, address dexFactory, uint24 poolFee)
 private view returns (bytes32 salt, address predicted) {
 salt = startSalt;
 predicted = _computeCreate2Address(salt, initCodeHash);
 if (predicted.code.length != 0 || IUniswapV3FactoryLike(dexFactory).getPool(predicted, pairToken, poolFee) != address(0)) revert TokenDeploymentFailed();
 }
 ...
 || config.poolFee != 10_000 || config.tickSpacing != 200
 ) {
